...
<button pButton
        data-testid="cust-search-query-btn"
        label="查詢"
        (click)="onQuery()">
</button>
...
> 建議在專案中建立 `testid-prefix.constants.ts` 常數檔，統一管理模組縮寫。  
> 被測專案與測試專案各放一份，修改時須同步更新。

```typescript
// testid-prefix.constants.ts
...
<input pInputText
       ...
       aria-label="電子郵件"
       aria-describedby="cust-form-email-error"
       formControlName="email" />
<small id="cust-form-email-error"
       data-testid="cust-form-email-error-text"
...
3. **PrimeNG 內部生成的 CSS class（如 `.ui-dropdown-item`）僅限在 Playwright 測試端使用**，用於從 `data-testid` 錨點往內部定位操作元素；應用程式碼本身不應依賴這些 class

> **注意**：PrimeNG 內部 CSS class 可能隨版本升級改變。若升級 PrimeNG 版本，須重新驗證附錄 A 的對照表。
...
1. 元件是否接受 `dataTestId` 的 `@Input`
2. `data-testid` 是否綁定到**實際互動元素**上（而非外層裝飾容器）
3. 若元件有多個互動元素，是否用後綴區分（如 `-input`、`-btn`）
4. 若包含表單控制項，是否支援 `ariaLabel` 的 `@Input`
5. 元件內部的狀態（展開、選取、停用）是否有對應的 ARIA 狀態屬性
...
> **注意**：表中 CSS class（如 `.ui-dropdown-item`）為 PrimeNG 8.1.1 版本，升級版本後須重新驗證。
...
| 版本 | 日期 | 變更說明 |
|------|------|---------|
| v1.0 | 2026/02/24 | 初版建立 |
| v1.1 | 2026/02/24 | 修正範例命名一致性、修正動態綁定語法、補充 PrimeNG CSS class 使用說明、新增變更紀錄 |
