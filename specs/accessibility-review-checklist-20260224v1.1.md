...
- [ ] **3.5** `aria-label` 的值是否使用有意義的描述（而非重複 `data-testid` 的值）？
...
- [ ] **5.3** `p-table` / `p-treeTable` 的 body template 中，`<tr>` 是否有動態 `[attr.data-testid]`？
...
- [ ] **8.3** 測試是否**未依賴** CSS class 或 DOM 層級結構定位？（PrimeNG 內部 class 除外，參見規範 §7.1）
...
6. 若有 PrimeNG 元件 → 對照規範附錄 A 確認標註方式
...
| v1.1 | 2026/02/24 | 同步規範文件版本、補充 §5.3 動態綁定語法、§8.3 PrimeNG class 例外說明、新增變更紀錄 |

---

> **備註**：此檢查表應與 `accessibility-testing-standards-20260224v1.1.md` 一同維護。當規範新增規則時，同步更新本表。
